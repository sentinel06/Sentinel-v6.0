...
- **Per-Tenant Data Isolation**: `audit_logs.owner_user_id` (nullable, indexed) carries the Clerk userId of whoever owns each row. Stamped on insert in `routes/logs.ts` (POST /v1/log) and `routes/gateway.ts` (registration + telemetry) via `lib/owner.ts:resolveOwnerFromKey()` (looks up `X-Sentinel-Key` in `partner_keys`). Read endpoints (`/v1/logs`, `/v1/logs/:id`, `/v1/traces/:traceId`, `/v1/stats`, `/v1/agents`, `/v1/compliance/export`, `/v1/forensic/*`) compose `viewerScopeCondition(req)` from `lib/owner.ts` into their WHERE clause. Rule: admin → `owner_user_id IS NOT NULL` (every real tenant, never the demo slice); signed-in viewer → `owner_user_id = userId`; **anonymous viewer → `false` (no rows — visitors must sign up to connect an agent)**. The marketing landing page is pure copy and does not hit any tenant-scoped endpoint. Per-user keys are issued by `routes/me.ts` via `POST /v1/me/key` (idempotent; reuses `partner_keys` with `partnerId = clerk userId`). System metrics that describe the platform itself (integrity, swarm map, status, whitepaper, pulse) intentionally remain global.
